...
int write_command(kis_capture_handler_t *caph, local_catsniffer_t *catsniffer, uint8_t *cmd, size_t len) {
    ...
    if (write(catsniffer->fd, (unsigned char *) cmd, fz_cmd_len) != fz_cmd_len) {
        ...
        printf("capture_thread error failed to send command - setting spindown\n");
...
void capture_thread(kis_capture_handler_t *caph) {
    ...
    while (1) {
        ...
        const size_t  frame_len = sizeof(fz_hdr) + pkt_len;
        printf("Received frame (chan=%u, %zu bytes): ", (unsigned)chan, frame_len + 1);
        printf("%02X ", chan);
    ...
    printf("capture_thread loop terminated; sending spindown\n");
    cf_handler_spindown(caph);
}
...
int main(int argc, char *argv[]) {
    ...
    //printf("Calling cf_handler_init(\"catsniffer_zigbee\").\n"); //Debug
