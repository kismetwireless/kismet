...
// Forward decls to keep header light (definitions are in packet.h, etc.)
class kis_packet;
class kis_datachunk;
...
public:
    kis_datasource_catsniffer_zigbee(shared_datasource_builder in_builder) :
        ...
        // Set the capture binary and DLT
...
protected:
    int handle_rx_data_content(kis_packet *packet, kis_datachunk *datachunk,
                               const uint8_t *content, size_t content_sz) override;

    int pack_comp_decap, pack_comp_radiodata, pack_comp_linkframe;
};
...
public:
    ...
    virtual void initialize() override {
        ...
        fprintf(stderr, "Debug: List capability set to false\n");
...
#endif // __DATASOURCE_CATSNIFFER_ZIGBEE_H__
