...
int ticc2531_set_channel(kis_capture_handler_t *caph, uint8_t channel) {
    /* printf("channel %u\n", channel); */
printf("ticc2531_set_channel channel:%u\n",channel);
...
printf("ticc2531_set_power\n");
    ...
    pthread_mutex_unlock(&(localticc2531->usb_mutex));
printf("ticc2531_set_power ret:%d return\n",ret);
    return ret;
}//mutex inside

int ticc2531_enter_promisc_mode(kis_capture_handler_t *caph) {
printf("ticc2531_enter_promisc_mode\n");
    int ret;
    local_ticc2531_t *localticc2531 = (local_ticc2531_t *) caph->userdata;
    pthread_mutex_lock(&(localticc2531->usb_mutex));
...
printf("channel->channel:%d localticc2531->channel:%d\n",channel->channel,localticc2531->channel);
...
printf("capture_thread\n");
    ...
    while (1) {
        ...
        printf("ticc2531_receive_payload buf_rx_len:%d\n",buf_rx_len);
        if (buf_rx_len < 0) {
