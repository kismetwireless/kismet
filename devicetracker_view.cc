...
unsigned int DevicetrackerView::device_endpoint_handler(std::ostream& stream, 
    ...
    // Wrapper object name, if any, used by datatables
    auto wrapper_name = std::string{};
    ...
    try {
        ...
        // If we have a wrapper, apply that
        wrapper_name = structured->getKeyAsString("wrapper", "");
    ...
    std::shared_ptr<TrackerElementStringMap> wrapper;

    // Field we transmit in the final stage (dervied array, or map)
    std::shared_ptr<TrackerElement> transmit;

    // Datatables specific elements added to the wrapper in DT mode
    auto dt_length_elem = std::make_shared<TrackerElementUInt64>();
    auto dt_start_elem = std::make_shared<TrackerElementUInt64>();
