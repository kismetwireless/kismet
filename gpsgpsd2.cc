...
kis_gps_gpsd_v2::kis_gps_gpsd_v2(shared_gps_builder in_builder) : 
    ...
    error_reconnect_timer = 
        timetracker->register_timer(SERVER_TIMESLICES_SEC * 10, NULL, 1,
                [this](int) -> int {
                {
                    local_shared_locker l(&gps_mutex, "gps reconnect timer");
...
bool kis_gps_gpsd_v2::open_gps(std::string in_opts) {
    local_locker lock(&gps_mutex, "gpsd open");
...
bool kis_gps_gpsd_v2::get_location_valid() {
    local_shared_locker lock(&gps_mutex, "gpsd get location");
...
void kis_gps_gpsd_v2::gpsd_io() {
    ...
    while (1) {
        try {
                ...
                if (inptok[it][0] == '{') {
                    ...
                    try {
                        ...
                        if (msg_class == "VERSION") {
                            ...
                            _MSG("GPSD connected to a JSON-enabled GPSD version " +
                    ...
                    if (pollvec[1].substr(0, 2) == "P=" && 
                            sscanf(pollvec[1].c_str(), "P=%lf %lf", 
                                &(new_location->lat), &(new_location->lon)) != 2) {
                        continue;
                    }

                    if (pollvec[4].substr(0, 2) == "M=" && 
                            sscanf(pollvec[4].c_str(), "M=%d", &(new_location->fix)) != 1) {
                        continue;
                    }

                    if (pollvec[2].substr(0, 2) == "A=" && 
                            sscanf(pollvec[2].c_str(), "A=%lf", &(new_location->alt)) != 1)
                        set_alt = false;
                    else
                        set_alt = true;

                    if (pollvec[3].substr(0, 2) == "V=" && 
                            sscanf(pollvec[3].c_str(), "V=%lf", &(new_location->speed)) != 1) {
                        set_speed = false;
                    } else  {
                        set_speed = true;

                        // Convert from knots to kph - unclear if truly knots still but 
                        // lets hope; this is only in an ancient gpsd
            ...
            // If we've gotten this far in the parser, we've gotten usable data, even 
            // if it's not actionable data (ie status w/ no valid signal is OK, but 
            // mangled unparsable nonsense isn't.)
            last_data_time = time(0);
            ...
            _MSG_ERROR("GPS did not get any data from GPSD server in 30 seconds, disconnecting.");
