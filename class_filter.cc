...
unsigned int ClassfilterMacaddr::edit_endp_handler(std::ostream& stream, 
    ...
    try {
        ...
        std::shared_ptr<TrackerElementMacMap> target = filter_block;
...
bool ClassfilterMacaddr::filter(mac_addr mac) {
    auto si = filter_block->find(mac);
    if (si != filter_block->end()) {
