...
void StorageMsgpackAdapter::Packer(GlobalRegistry *globalreg, SharedTrackerElement v,
    ...
    // on/objectname, original kismet name of the object
    o.pack("on");
    o.pack(globalreg->entrytracker->GetFieldName(v->get_id()));

    // ot/objecttype, original kismet type of the object
    o.pack("ot");
    o.pack(TrackerElement::type_to_typestring(v->get_type()));

    // od/objectdata, actual content
    o.pack("od");
    ...
    switch (v->get_type()) {
        ...
        case TrackerMap:
            ...
            for (map_iter = tmap->begin(); map_iter != tmap->end(); ++map_iter) {
                ...
                StorageMsgpackAdapter::Packer(globalreg, map_iter->second, o, name_map);
            }
            break;
        case TrackerIntMap:
            tintmap = v->get_intmap();
            ...
            for (int_map_iter = tintmap->begin(); int_map_iter != tintmap->end(); 
                    ++int_map_iter) {
                o.pack(int_map_iter->first);
                StorageMsgpackAdapter::Packer(globalreg, int_map_iter->second, o, name_map);
            }
            break;
        case TrackerMacMap:
            tmacmap = v->get_macmap();
            ...
            for (mac_map_iter = tmacmap->begin(); 
                ...
                // macmaps sent as macaddr only
                o.pack(mac_map_iter->first.Mac2String());
                StorageMsgpackAdapter::Packer(globalreg, mac_map_iter->second, o, name_map);
            }
            break;
        case TrackerStringMap:
            tstringmap = v->get_stringmap();
            ...
            for (string_map_iter = tstringmap->begin();
                    ...
                    ++string_map_iter) {
                o.pack(string_map_iter->first);
                StorageMsgpackAdapter::Packer(globalreg, string_map_iter->second, o, name_map);
            }
            break;
        case TrackerDoubleMap:
            tdoublemap = v->get_doublemap();
            ...
            for (double_map_iter = tdoublemap->begin();
                    ...
                    ++double_map_iter) {
                o.pack(double_map_iter->first);
                StorageMsgpackAdapter::Packer(globalreg, double_map_iter->second, o, name_map);
            }
            break;
        case TrackerByteArray:
            bytes = v->get_bytearray();
...
void StorageMsgpackAdapter::Pack(GlobalRegistry *globalreg, std::ostream &stream,
        SharedTrackerElement e, TrackerElementSerializer::rename_map *name_map) {
    msgpack::packer<std::ostream> packer(&stream);
    StorageMsgpackAdapter::Packer(globalreg, e, packer, name_map);
}
