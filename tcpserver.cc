...
int TcpServer::Setup(unsigned int in_max_clients, short int in_port) {
    ...
    if (serv_fd > (unsigned int) max_fd)
        max_fd = serv_fd;
...
int TcpServer::SetIPFilter(vector<client_ipfilter *> in_filter) {
    ipfilter_vec = in_filter;
